...
2. `WTY5` envelope：已验证联系人消息优先使用 Double Ratchet；响应方首条前可回退 WTY4 session-key 路径并必须对用户可见。
...
- macOS/Windows/Linux 第一版只承诺普通文本输入和 `WTY4:` 密文文本直接插入当前光标/选区；联系人 WTY5 需要平台侧身份/联系人状态接入后再启用。
